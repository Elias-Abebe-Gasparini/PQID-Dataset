# Phase 3 Recall Expansion Cells

Paste these code cells into `PQID/scripts/scrape_github_unified.ipynb` after the current Phase 2 merge cell.

These cells are designed to:

- leave Cells 1–10 and the Phase 2 cells unchanged
- treat Phase 3 as a separate recall-expansion campaign
- write Phase 3 results to separate files
- merge baseline + Phase 2 + Phase 3 into a final raw pool
- preserve explicit retrieval metadata and campaign-specific manifests

Assumption: the original notebook cells and the Phase 2 cells have already been run, so variables and helpers such as `BASE`, `OUTPUT_FILE`, `OUTPUT_FILE_V2`, `API_BASE`, `SCRAPE_DATE`, `session`, `api_get`, `SEARCH_SLEEP`, `load_processed`, `load_seen_hashes`, `load_jsonl_any`, `append_jsonl_to`, `SCHEMA_NULL_FIELDS`, `extract_circuits_python_aggressive`, and `extract_circuits_notebook_aggressive` already exist in memory.

## Cell 19

```python
# Cell 19 — Phase 3 config, manifest, and isolated state
from collections import Counter

PHASE3_LABEL = "aggressive_v2"
...
MAX_FILE_SIZE_BYTES_V3 = 2_000_000
INCLUDE_DOC_PATHS_V3 = True
SEARCH_PAGE_LIMIT_V3 = 10
...
# Frozen, manually reviewed candidates derived from Phase 2 empirical analysis.
PHASE3_EMPIRICAL_PROMOTED_REPOS = [
    ("runtsang", "Q-Bridge"),
...
# Tutorial-heavy repos worth a second, more permissive pass.
PHASE3_HIGH_YIELD_RESWEEP_REPOS = [
    ("Qiskit", "documentation"),
    ("sethuquantum", "LearnQuantum"),
    ("AayushSarkar", "Qiskit-Experiment-Hub"),
    ("AIComputing101", "quantum-computing-101"),
...
# Selective owner sweeps; keep this frozen for reproducibility.
PHASE3_OWNER_SWEEPS = [
    "runtsang",
    "sethuquantum",
    "AayushSarkar",
...
PHASE3_SEARCH_QUERIES_V2 = list(dict.fromkeys([
    ...
    "from qiskit.circuit.library import EvolvedOperatorAnsatz language:python",
    "QuantumCircuit.from_qasm_str( qiskit language:python",
    ".to_instruction( qiskit language:python",
    ".control( qiskit language:python",
    "measure_all() qiskit language:python path:tutorial",
    ...
    "from qiskit_nature language:python QuantumCircuit",
]))

PHASE3_NOTEBOOK_QUERY_PACK = list(dict.fromkeys([
    "qiskit language:jupyter-notebook path:notebooks",
    "qiskit language:jupyter-notebook path:tutorial",
    "qiskit language:jupyter-notebook path:examples",
...
def write_manifest_v3(extra=None):
    ...
    manifest = {
        ...
        "empirical_promoted_repos_v3": PHASE3_EMPIRICAL_PROMOTED_REPOS,
        "high_yield_resweep_repos_v3": PHASE3_HIGH_YIELD_RESWEEP_REPOS,
        "owner_sweeps_v3": PHASE3_OWNER_SWEEPS,
...
print(f"Phase 3 output      : {repo_rel_display(OUTPUT_FILE_V3)}")
print(f"Phase 3 processed   : {repo_rel_display(PROCESSED_FILE_V3)}")
print(f"Phase 3 merged file : {repo_rel_display(MERGED_FILE_V3)}")
...
```python
# Cell 20 — Phase 3 helper wrappers
...
def get_owner_repos_v3(session, owner):
    repos = []
...
```python
# Cell 21 — Strategy D: empirical promoted repos from Phase 2 analysis
total_empirical_v3 = 0
empirical_counts_v3 = {}

for i, (owner, repo) in enumerate(PHASE3_EMPIRICAL_PROMOTED_REPOS, 1):
    n = process_repo_v3(
        session, owner, repo,
        source_tag="empirical_promoted_repo_v3",
    ...
    empirical_counts_v3[f"{owner}/{repo}"] = n
    total_empirical_v3 += n
    print(f"[{i}/{len(PHASE3_EMPIRICAL_PROMOTED_REPOS)}] {owner}/{repo} -> +{n}")

print(f"\nEmpirical promoted repo total: {total_empirical_v3:,}")
```

## Cell 22

```python
# Cell 22 — Strategy E: relaxed re-sweep of tutorial-heavy high-yield repos
total_resweep_v3 = 0
resweep_counts_v3 = {}

for i, (owner, repo) in enumerate(PHASE3_HIGH_YIELD_RESWEEP_REPOS, 1):
    n = process_repo_v3(
        session, owner, repo,
        source_tag="relaxed_resweep_v3",
        retrieval_strategy="relaxed_resweep",
        seen_hashes=seen_hashes_v3,
        processed=processed_v3,
        processed_file=PROCESSED_FILE_V3,
        output_file=OUTPUT_FILE_V3,
    ...
    total_resweep_v3 += n
    print(f"[{i}/{len(PHASE3_HIGH_YIELD_RESWEEP_REPOS)}] {owner}/{repo} -> +{n}")

print(f"\nRelaxed re-sweep total: {total_resweep_v3:,}")
```

## Cell 23

```python
# Cell 23 — Strategy F: expanded search set v2
...
```python
# Cell 24 — Strategy G: selective owner sweeps
total_owner_v3 = 0
owner_repo_counts_v3 = {}

for owner in PHASE3_OWNER_SWEEPS:
    repo_names = get_owner_repos_v3(session, owner)
    owner_repo_counts_v3[owner] = len(repo_names)
    print(f"{owner}: {len(repo_names)} public repos")

    for i, repo_name in enumerate(repo_names, 1):
        n = process_repo_v3(
            session, owner, repo_name,
            source_tag="owner_sweep_v3",
            retrieval_strategy="owner_sweep",
            seen_hashes=seen_hashes_v3,
            processed=processed_v3,
            processed_file=PROCESSED_FILE_V3,
            output_file=OUTPUT_FILE_V3,
        ...
        total_owner_v3 += n
        if n:
            print(f"  [{i}/{len(repo_names)}] {owner}/{repo_name} -> +{n}")

print(f"\nSelective owner sweep total: {total_owner_v3:,}")
```

## Cell 25

```python
# Cell 25 — Strategy H: notebook-heavy query pack
...
print(f"Empirical repos added       : {total_empirical_v3:,}")
print(f"Relaxed re-sweeps added     : {total_resweep_v3:,}")
print(f"Expanded search v2 added    : {total_search_v3:,}")
print(f"Owner sweeps added          : {total_owner_v3:,}")
...
write_manifest_v3({
    ...
    "empirical_repo_total": total_empirical_v3,
    "resweep_total": total_resweep_v3,
    "search_v2_total": total_search_v3,
    "owner_sweep_total": total_owner_v3,
...
print(f"\nMerged file written to: {repo_rel_display(MERGED_FILE_V3)}")
print(f"Manifest written to   : {repo_rel_display(MANIFEST_FILE_V3)}")
```

## Downstream Note

Before seed generation, run `PQID/scripts/enrich_raw_circuits.py` on the final merged raw pool so the Phase 3 retrieval metadata and structural circuit metrics both propagate into the seed-generation stage.
...
After that, `PQID/scripts/03_instruction_generation/generate_seeds.py` will auto-detect `circuits_unified_plus_phase2_plus_phase3_enriched.jsonl` as its default input. You can still pass `--input-file` explicitly if you want to pin the run to a specific artifact.

That way:

- baseline entries retain `retrieval_mode="baseline"`
- Phase 2 and Phase 3 entries retain `retrieval_mode="aggressive"`
- all campaigns keep explicit `retrieval_strategy` and `retrieval_run_id`
- seed prompts can use the richer circuit-structure anchors from the fully expanded raw pool
